...
### Round 3 (DRY refactor + TS cleanup)
14. `src/lib/tool-display.ts` -- NEW shared utility: TOOL_LABELS, TOOL_COLORS, getToolLabel(), getToolColor(). Single source of truth for all tool display names and badge colors.
15. 12 files updated to import from `@/lib/tool-display` instead of local declarations: analytics, targets, targets/[id], playbooks, practice, requests, requests/[id], dashboard, admin, demo/[toolName], WinLossIntelligence, email.ts. Also fixed 4 stale files (admin, demo, dashboard, email) that were missing new tool entries.
16. `src/app/api/requests/route.ts` -- added explicit `any` types to `$transaction` and `.catch` params (TS strict mode fix)
17. `src/app/api/batch-requests/route.ts` -- same TS strict mode fix
...
### Tool Name Display Mapping -- FIXED + REFACTORED
~~Multiple pages display raw tool IDs. A shared utility would fix this everywhere.~~
Fixed by adding missing entries, then extracted to shared `src/lib/tool-display.ts`. All 12 consumer files now import from single source of truth. Adding a new tool = 1 file change instead of 12.
