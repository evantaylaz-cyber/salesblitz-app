...
const STATUS_COLOR: Record<string, { text: string; bg: string }> = {
  submitted: { text: "text-blue-400", bg: "bg-blue-500/10 border-blue-500/20" },
  researching: { text: "text-amber-400", bg: "bg-amber-500/10 border-amber-500/20" },
  processing: { text: "text-amber-400", bg: "bg-amber-500/10 border-amber-500/20" },
  awaiting_clarification: { text: "text-amber-400", bg: "bg-amber-500/10 border-amber-500/20" },
  ready: { text: "text-emerald-400", bg: "bg-emerald-500/10 border-emerald-500/20" },
  delivered: { text: "text-emerald-400", bg: "bg-emerald-500/10 border-emerald-500/20" },
  completed: { text: "text-emerald-400", bg: "bg-emerald-500/10 border-emerald-500/20" },
  failed: { text: "text-red-400", bg: "bg-red-500/10 border-red-500/20" },
};
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a]">
      ...
      <main className="mx-auto max-w-3xl px-6 py-8 space-y-8">
        ...
        <div className="rounded-xl border bg-[#141414] p-6 shadow-sm shadow-black/20 space-y-4">
          <div className="flex items-start justify-between">
            <div>
              <h2 className="text-lg font-semibold text-white">Batch Status</h2>
              <p className="text-sm text-neutral-400 mt-1">{batchJob.totalAccounts} account{batchJob.totalAccounts !== 1 ? "s" : ""} in batch</p>
            </div>
          ...
          {isAwaiting && (
            <div className="flex items-start gap-3 rounded-lg border border-amber-500/20 bg-amber-500/10 p-4">
              ...
              <div>
                <p className="text-sm font-medium text-amber-400">Clarification Needed</p>
                <p className="text-sm text-amber-400 mt-1">We need more information to complete some accounts. Check your email for a clarification request.</p>
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <div className="h-2 rounded-full bg-[#1a1a1a] overflow-hidden">
              <div className={`h-full rounded-full transition-all duration-500 ${
                batchJob.status === "failed" ? "bg-red-500/100"
                : batchJob.percentComplete === 100 ? "bg-emerald-500/100"
                : isAwaiting ? "bg-amber-500/100"
                : "bg-emerald-500/100"
          ...
          <div className="grid grid-cols-3 gap-4 pt-2">
            ...
            <div className="rounded-lg bg-[#0a0a0a] p-3">
              <p className="text-xs text-neutral-400 mb-1">{(batchJob.awaitingAccounts || 0) > 0 ? "Awaiting Input" : "In Progress"}</p>
              <p className={`text-lg font-semibold ${(batchJob.awaitingAccounts || 0) > 0 ? "text-amber-400" : "text-amber-400"}`}>
                {batchJob.totalAccounts - batchJob.completedAccounts - batchJob.failedAccounts}
              </p>
            </div>
            <div className="rounded-lg bg-[#0a0a0a] p-3">
              <p className="text-xs text-neutral-400 mb-1">Failed</p>
              ...
              .map((step, idx) => (
                <div key={step.name || step.id || idx}>
                  ...
                  <div className="h-1.5 rounded-full bg-[#1a1a1a] overflow-hidden">
                    <div className={`h-full rounded-full transition-all duration-500 ${
                      step.status === "completed" ? "bg-emerald-500/100 w-full"
                      : step.status === "in_progress" ? "bg-emerald-500/100 w-1/2"
                      : step.status === "failed" ? "bg-red-500/100 w-full"
                      : "bg-[#262626] w-0"
                    }`} />
                  </div>
                </div>
              ...
              batchJob.accountStatuses.map((account) => {
                ...
                return (
                  <button key={account.requestId}
                    ...
                    className={`w-full text-left rounded-lg border px-4 py-3 transition hover:border-emerald-500/30 ${statusColor.bg}`}>
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-neutral-400">{account.targetCompany}</p>
                        {account.status === "awaiting_clarification" && (
                          <p className="text-xs text-amber-400 mt-1.5">Check email for clarification request</p>
                        )}
                        {account.currentStep && (<p className="text-xs text-neutral-500 mt-1.5">{account.currentStep}</p>)}
                      </div>
        ...
        {isComplete && batchJob.synthesisHighlights && (
          <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-6 space-y-4">
            <div className="flex items-center gap-2">
              <TrendingUp className="h-5 w-5 text-emerald-400" />
              <h3 className="text-sm font-semibold text-emerald-400">Synthesis Highlights</h3>
            </div>
            {(batchJob.synthesisHighlights.topPriorityAccount || batchJob.synthesisHighlights.priorityRanking) && (
              <div>
                <p className="text-xs text-emerald-400 uppercase tracking-wide mb-1">Top Priority Account</p>
            ...
            {(batchJob.synthesisHighlights.territoryStrategySummary || batchJob.synthesisHighlights.keyInsight) && (
              <div>
                <p className="text-xs text-emerald-400 uppercase tracking-wide mb-1">Territory Strategy</p>
            ...
            <div className="space-y-2">
              {batchJob.batchAssetUrls.map((asset: any, idx: number) => (
                <a key={idx} href={asset.url} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-3 rounded-lg border bg-[#0a0a0a] px-4 py-3 hover:bg-[#1a1a1a] transition">
