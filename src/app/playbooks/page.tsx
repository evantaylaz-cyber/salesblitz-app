...
import { TOOL_LABELS, TOOL_COLORS, getToolColor } from "@/lib/tool-display";
import {
  Loader2,
  ExternalLink,
  Shield,
        ...
        <div className="mb-6">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-[#141414] border border-[#262626] rounded-lg text-sm text-white placeholder-neutral-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
            />
          </div>
        </div>
        ...
        {error ? (
          ...
          <div className="rounded-xl border border-[#262626] bg-[#141414] p-12 text-center">
            ...
            {!searchQuery && (
              <a
                href="/request"
                className="inline-flex items-center gap-2 mt-4 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-medium hover:bg-emerald-600 transition-colors"
              >
                <Zap className="h-4 w-4" />
                New Request
              </a>
...
function CompanyCard({ group }: { group: CompanyGroup }) {
  ...
  return (
    <div className="bg-[#141414] rounded-xl border border-[#262626] overflow-hidden hover:border-[#333333] transition-colors">
      {/* Company header */}
        ...
        <div className="flex items-center gap-2">
          {latestPlaybook?.playbookUrl && (
            <a
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 text-white rounded-lg text-sm font-medium hover:bg-emerald-600 transition-colors"
            >
      ...
      {expanded && (
        <div className="border-t border-[#1a1a1a] divide-y divide-[#1a1a1a]">
          {group.runs.map((run) => (
            <div
              key={run.id}
              className="px-5 py-3 flex items-center justify-between bg-[#0a0a0a]/50"
            >
              <div className="flex items-center gap-3">
                <span
                  className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                    TOOL_COLORS[run.toolName] || "bg-[#1a1a1a] text-neutral-300"
                  }`}
                >
                ...
                <span className="text-sm text-neutral-200">
                  {run.targetName}
                  {run.targetRole ? (
                    <span className="text-neutral-500">, {run.targetRole}</span>
                  ) : null}
                </span>
